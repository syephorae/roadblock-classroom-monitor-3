...
export async function GET() {
  try {
    ...
    const userIds = studentsSnapshot.docs.map(doc => {
      const data = doc.data();
      return {
    ...
    console.error('Error fetching user IDs by time:', error);
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
    return new NextResponse(JSON.stringify({ error: 'Failed to fetch user IDs by time', details: errorMessage }), { status: 500 });
  }
}
